...
description: UTan – Full Android replica of iOS version (Complete)
publish_to: 'none'
version: 3.0.6+11
...
# ==================== 5. Models ====================
# 5.1 video_item.dart
video_item_content = '''class VideoItem {
  final String id;
  final String title;
...
with open(f"{BASE}/lib/models/video_item.dart", "w", encoding="utf-8") as f:
    f.write(video_item_content)

# 5.2 episode.dart
episode_content = '''class EpisodeItem {
  final String id;
  final String title;
  final String url;
...
# 5.3 media_details.dart
media_details_content = '''import 'episode.dart';
class MediaDetails {
...
with open(f"{BASE}/lib/models/media_details.dart", "w", encoding="utf-8") as f:
    f.write(media_details_content)

# 5.4 watch_progress.dart
watch_progress_content = '''class WatchProgress {
...
with open(f"{BASE}/lib/models/watch_progress.dart", "w", encoding="utf-8") as f:
    f.write(watch_progress_content)

# 5.5 download_task.dart
download_task_content = '''class DownloadTaskItem {
...
with open(f"{BASE}/lib/models/download_task.dart", "w", encoding="utf-8") as f:
    f.write(download_task_content)

# 5.6 site_category.dart
site_category_content = '''class SiteCategory {
...
with open(f"{BASE}/lib/models/site_category.dart", "w", encoding="utf-8") as f:
    f.write(site_category_content)

# ==================== 6. Services ====================
# 6.1 scraper.dart (مع جميع الإصلاحات)
scraper_content = '''import 'dart:async';
import 'dart:io';
    ...
    for (final match in epBlock.allMatches(html)) {
      ...
      if (url.isNotEmpty) {
        episodes.add(EpisodeItem(
          ...
          url: url, url1080: url1080, url360: url360,
          subtitleUrl: srt, subtitleVttUrl: vtt,
        ));
      }
    }
...
class DownloadManager {
  ...
  Future<void> startDownload({
    ...
    final task = DownloadTaskItem(
      id: id, title: title, imageUrl: imageUrl, isMovie: isMovie,
      videoUrl: videoUrl, subtitleUrl: subtitleUrl,
    );
                      ...
                      ...d.seasonsDict[_selectedSeason]!.map((ep) => Container(
                        ...
                        child: ListTile(
                          ...
                          trailing: IconButton(
                            ...
                            onPressed: () => DownloadManager().startDownload(
                              id: ep.id, title: ep.title, imageUrl: d.imageUrl, isMovie: false,
                              videoUrl: ep.url, subtitleUrl: ep.subtitleUrl,
                            ),
                          ),
...
# 8.9 player_screen.dart (الملف الأخير)
...
print("✅ تم إنشاء المشروع الكامل غير المنقوص في المجلد: " + BASE)
print("📁 جميع الملفات التالية تم إنشاؤها:")
print("   - pubspec.yaml, android/.../network_security_config.xml, android/.../AndroidManifest.xml")
print("   - lib/main.dart")
print("   - lib/models/: video_item.dart, episode.dart, media_details.dart, watch_progress.dart, download_task.dart, site_category.dart")
print("   - lib/services/: scraper.dart, subtitle_parser.dart, progress_store.dart, download_manager.dart, favorites_store.dart, settings_store.dart")
print("   - lib/widgets/: poster_card.dart, hero_banner.dart, continue_row.dart, category_row.dart")
print("   - lib/screens/: home_screen.dart, browse_screen.dart, category_list_screen.dart, search_screen.dart, downloads_screen.dart, settings_screen.dart, history_screen.dart, details_screen.dart, player_screen.dart")
print("🚀 الآن شغّل:")
print(f"   cd {BASE}")
print("   flutter clean")
